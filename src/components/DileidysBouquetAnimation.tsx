...
export const DileidysBouquetAnimation: React.FC<DileidysBouquetAnimationProps> = ({
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => setStep('yellow-foundation'), 2200);
    const t2 = setTimeout(() => setStep('signature-rose-peony'), 4800);
    const t3 = setTimeout(() => setStep('yellow-canary-dahlia'), 7400);
    const t4 = setTimeout(() => setStep('center-accents'), 10000);
    const t5 = setTimeout(() => {
      setStep('bouquet-complete');
      setIsCompleted(true);
    }, 12800);
  ...
  const handleReplay = () => {
    ...
    setIsCompleted(false);
    setStep('stems-wrap');
    setTimeout(() => setStep('yellow-foundation'), 2200);
    setTimeout(() => setStep('signature-rose-peony'), 4800);
    setTimeout(() => setStep('yellow-canary-dahlia'), 7400);
    setTimeout(() => setStep('center-accents'), 10000);
    setTimeout(() => {
      setStep('bouquet-complete');
      setIsCompleted(true);
    }, 12800);
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-3">
          {isCompleted && (
            <>
              ...
              {onBackToReading && (
                <button
                  id="btn-dileidys-view-message"
                  type="button"
                  onClick={onBackToReading}
                  className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-full border border-[#FB7185]/40 hover:border-[#F43F5E] bg-[#FFF1F2]/10 hover:bg-[#FFF1F2]/20 text-[#FFE4E6] text-xs transition-colors cursor-pointer"
                  title="Mensaje"
                >
                  <BookOpen className="w-3.5 h-3.5 text-[#FFE4E6]" />
                  <span>Mensaje</span>
                </button>
              )}
              ...
              <button
                id="btn-dileidys-replay-bouquet"
                type="button"
                onClick={handleReplay}
                className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-full border border-[#9F1239]/40 hover:border-[#F43F5E] bg-[#2A040E]/70 hover:bg-[#4C0519]/80 text-[#FECDD3] text-xs transition-colors cursor-pointer"
