...
export const GenesisBouquetAnimation: React.FC<GenesisBouquetAnimationProps> = ({
  ...
  useEffect(() => {
    ...
    // Natural progressive manual assembly sequence starting directly from the bouquet base & stems
    const t1 = setTimeout(() => setStep('background-yellows'), 2200);
    const t2 = setTimeout(() => setStep('side-blooms'), 4800);
    const t3 = setTimeout(() => setStep('signature-iris'), 7400);
    const t4 = setTimeout(() => setStep('heart-rose-accents'), 10000);
    const t5 = setTimeout(() => {
      setStep('bouquet-complete');
      setIsCompleted(true);
    }, 12800);
  ...
  const handleReplay = () => {
    ...
    setIsCompleted(false);
    setStep('stems-wrap');
    setTimeout(() => setStep('background-yellows'), 2200);
    setTimeout(() => setStep('side-blooms'), 4800);
    setTimeout(() => setStep('signature-iris'), 7400);
    setTimeout(() => setStep('heart-rose-accents'), 10000);
    setTimeout(() => {
      setStep('bouquet-complete');
      setIsCompleted(true);
    }, 12800);
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-3">
          {isCompleted && (
            <>
              {/* Enter User Response */}
              <motion.button
                id="btn-genesis-enter-response"
                type="button"
                onClick={onProceedToResponse}
                initial={{ opacity: 0, scale: 0.94 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.5 }}
                className="w-full sm:w-auto inline-flex items-center justify-center space-x-2.5 px-6 py-3 rounded-full bg-gradient-to-r from-[#1D4E89] via-[#2563EB] to-[#F59E0B] hover:from-[#2563EB] hover:to-[#FBBF24] text-white text-xs font-semibold tracking-wider uppercase shadow-[0_0_24px_rgba(29,78,137,0.35)] transition-all hover:scale-102 cursor-pointer"
              >
              ...
              {onBackToReading && (
                <button
                  id="btn-genesis-view-message"
                  type="button"
                  onClick={onBackToReading}
                  className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-full border border-[#1D4E89]/30 hover:border-[#1D4E89] bg-[#FAF8F5]/80 hover:bg-[#FAF8F5] text-[#1D4E89] text-xs transition-colors cursor-pointer"
                  title="Mensaje"
                >
              ...
              <button
                id="btn-genesis-replay-bouquet"
                type="button"
                onClick={handleReplay}
                className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-full border border-[#D4C3A6] hover:border-[#1D4E89] bg-[#FAF8F5]/70 hover:bg-[#FAF7F2] text-[#64748B] hover:text-[#1D4E89] text-xs transition-colors cursor-pointer"
