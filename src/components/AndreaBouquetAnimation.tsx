...
export const AndreaBouquetAnimation: React.FC<AndreaBouquetAnimationProps> = ({
  ...
  useEffect(() => {
    ...
    // Andrea's bespoke assembly sequence: Yellow blooms base + Single Signature Orchid
    const t1 = setTimeout(() => setStep('yellow-upper'), 2200);
    const t2 = setTimeout(() => setStep('yellow-flanks'), 4800);
    const t3 = setTimeout(() => setStep('signature-orchid'), 7400);
  ...
  const handleReplay = () => {
    ...
    setTimeout(() => setStep('signature-orchid'), 7400);
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-3">
          {isCompleted && (
            <>
              ...
              {onBackToReading && (
                <button
                  id="btn-andrea-view-message"
                  type="button"
                  onClick={onBackToReading}
                  className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-full border border-[#BE123C]/30 hover:border-[#BE123C] bg-[#FAF4F6]/80 hover:bg-[#FAF4F6] text-[#BE123C] text-xs transition-colors cursor-pointer"
                  title="Mensaje"
                >
              ...
              <button
                id="btn-andrea-replay-bouquet"
                type="button"
                onClick={handleReplay}
                className="inline-flex items-center space-x-1.5 px-4 py-2.5 rounded-full border border-[#FDA4AF] hover:border-[#BE123C] bg-[#FAF4F6]/70 hover:bg-[#FCE7F3] text-[#64748B] hover:text-[#BE123C] text-xs transition-colors cursor-pointer"
